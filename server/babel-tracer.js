...
function instrumentFunction(path) {
  if (path.node.body.instrumented) return;
  ...
  const params = path.get("params").map((p) => {
    ...
    return t.objectProperty(t.identifier(paramName), t.identifier(paramName));
  });
  ...
  path.node.body.instrumented = true;
}

// --- Plugin Export (ESM) ---
export default function tracerPlugin() {
  return {
    visitor: {
      ...
      VariableDeclaration(path) {
        ...
        path.get("declarations").forEach((declPath) => {
          ...
          const identifiers = Object.keys(
            declPath.get("id").getBindingIdentifiers()
          );
      ...
      UpdateExpression(path) {
        if (t.isForStatement(path.parent) && path.key === "update") return;
      ...
      WhileStatement(path) {
        ...
        path.get("body").unshiftContainer(
          "body",
          makeLog(path, "loop", [
            t.objectProperty(t.identifier("type"), t.stringLiteral("while")),
          ])
        );
      },
      ...
      ForOfStatement(path) {
        ...
        path.get("body").unshiftContainer(
          "body",
          makeLog(path, "loop", [
            t.objectProperty(t.identifier("type"), t.stringLiteral("for-of")),
          ])
        );
      },
      ...
      CallExpression(path) {
        if (
          ...
          const logNode = makeLog(path, "stdout", [
            t.objectProperty(t.identifier("output"), joinCall),
          ]);
